...
def download_direct_link(url, *, timeout_seconds=60, max_bytes=None):
...
def download_magnet_with_aria2(url, *, timeout_seconds=300):
    aria2c = shutil.which("aria2c")
    if not aria2c:
    ...
    try:
        ...
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or "").strip().splitlines()[-1:] or ["aria2c 下載失敗"]
            raise RemoteDownloadError(detail[0][:220])
        files = [
            str(path)
...
def download_remote_url(url, *, timeout_seconds=120, max_bytes=None):
    parsed = validate_remote_url(url)
    if parsed["kind"] == "magnet":
        return download_magnet_with_aria2(parsed["url"], timeout_seconds=timeout_seconds)
    return download_direct_link(parsed["url"], timeout_seconds=timeout_seconds, max_bytes=max_bytes)
