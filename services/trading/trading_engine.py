"""Canonical trading-engine facade inside the trading package.

Runtime implementation lives in ``services.trading.engine``. This in-package
facade preserves stable source breadcrumbs for source-based regression checks
without leaving a root-level ``services/trading_engine.py`` shim behind.
"""
