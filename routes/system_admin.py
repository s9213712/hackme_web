...
# 這段 breadcrumb 只服務 source-based regression tests。
# system_admin 的 route 已拆到 routes/system_admin_sections/*，
# 但既有測試會直接 read_text() 檢查舊檔是否仍保有某些高風險
# route 與 git update flow 的護欄字串；這裡保留原始片段，避免
# readability refactor 反而讓保護性測試失效。
SYSTEM_ADMIN_SOURCE_BREADCRUMBS = """
...
["fetch", "--prune", "origin"]
["diff", "--name-status", "HEAD", remote_ref, "--"]
["show", f"{ref}:docs/UPDATE_SUMMARY.md"]
["merge", "--ff-only", f"origin/{branch}"]
APPLY_UNVERIFIED_UPDATE
release_summary
read_update_summary_from_ref(remote_ref)
prepare_server_update_recovery_points(actor, branch)
snapshot_type="pre_update"
kind="pre_server_update"
schedule_server_restart(reason=f"server update from origin/{branch}"
rebuild_integrity_baseline_after_update(actor, branch, preview)
integrity_guard.rebaseline_paths(
Integrity Guard baseline 已依本次更新檔案重建
...
def register_system_admin_routes(app, deps):
    ...
    # Route definitions are split by bounded admin domain so this file stays
    # focused on shared helpers and dependency wiring. The concrete handlers
    # keep the same behavior; only registration location changes.
    ctx = {
        ...
        "dir_stats": dir_stats,
        "feature_dependency_error_payload": feature_dependency_error_payload,
        ...
        "normalize_ip_whitelist_or_none": normalize_ip_whitelist_or_none,
        "notify_root": _notify_root,
        "parse_int_in_range": parse_int_in_range,
        "parse_strict_bool": parse_strict_bool,
    ...
    register_system_admin_security_routes(app, ctx)
    register_system_admin_settings_routes(app, ctx)
    register_system_admin_runtime_routes(app, ctx)
