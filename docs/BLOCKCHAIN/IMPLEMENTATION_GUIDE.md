...
2. **`docs/BLOCKCHAIN/`** 與 `docs/research/POINTSCHAIN_V2_BLOCKCHAINIZATION_PLAN.md`、`docs/research/POINTS_MINING_REWARDS_PLAN.md` 是 PointsChain v2 規格的一部分，可以**一起或分批** push，但要在 commit message 註明「設計拍板，尚未實作」狀態。
3. CI / pytest / pre-push 必須綠燈。若 #122 polling gap 還是 OPEN：在 commit message 註明，但仍可 push。
...
>
> **但是**：
> - 提出 → 等 root 拍板 → 才能寫進 `docs/BLOCKCHAIN/` 與動工
> - **不可** 自行修改 `docs/BLOCKCHAIN/` 既有取捨（除非是純補強而非取代）
...
root 拍板後，先改 `docs/BLOCKCHAIN/`、把 status 加上「root approved YYYY-MM-DD」，再動源碼。
...
#### 結論（2026-05-04 multi-role audit round 6 接受後）

> **🚫 BLOCK PHASE 1**
>
> 直到下列四件全部 **CLOSE 且重跑驗證通過**：
> - **#122**（30s polling gap）
> - **#135**（admin self-block）
> - **#136**（settings silent boolean coerce）
> - **#137**（settings range / format validation gap）
>
> 強烈建議在 Phase 0 一併修復（因牽涉 settings/security gate/wallet lifecycle，留到鏈化後成本更高）：
> **#138** wallet 500、**#139** IP whitelist、**#140** feature dependency。
>
> Low cleanup（不阻擋）：**#141** test scans .venv、**#142** prompt_password stdout。
...
| 🚫 BLOCKER | 4 | **#122**, **#135**, **#136**, **#137** |
| 🟡 RECOMMEND | 3 | **#138**, **#139**, **#140** |
| 🟡 PARTIAL | 2 | incident_lockdown 跨路徑 coverage / silent fallback 全站 audit |
| ⚪ LOW | 2 | **#141**, **#142** |
...
|---|---|---|---|---|
| 1 | [#122](https://github.com/s9213712/hackme_web/issues/122) | 🚫 BLOCKER | 30s polling 漏觸發 stop-loss / liquidation | [Block-1](PHASE_0_CLEANUP_GATE.md#block-1-github-122--30s-polling-漏觸發-stop-loss--liquidation-) |
| 2 | [#135](https://github.com/s9213712/hackme_web/issues/135) | 🚫 BLOCKER | Admin self-block：root 可自我封鎖 | [Block-2](PHASE_0_CLEANUP_GATE.md#block-2-github-135--admin-self-block-) |
| 3 | [#136](https://github.com/s9213712/hackme_web/issues/136) | 🚫 BLOCKER | settings silent boolean coerce | [Block-3](PHASE_0_CLEANUP_GATE.md#block-3-github-136--settings-silent-boolean-coerce-) |
| 4 | [#137](https://github.com/s9213712/hackme_web/issues/137) | 🚫 BLOCKER | settings range / format validation gap | [Block-4](PHASE_0_CLEANUP_GATE.md#block-4-github-137--settings-range--format-validation-gap-) |
| 5 | [#138](https://github.com/s9213712/hackme_web/issues/138) | 🟡 RECOMMEND | wallets/<id> 500 + traceback leak | [Recommend-5](PHASE_0_CLEANUP_GATE.md#recommend-5-github-138--walletsid-500--traceback-leak-) |
| 6 | [#139](https://github.com/s9213712/hackme_web/issues/139) | 🟡 RECOMMEND | IP whitelist accepts non-IP | [Recommend-6](PHASE_0_CLEANUP_GATE.md#recommend-6-github-139--ip-whitelist-accepts-non-ip-silent-skip-) |
| 7 | [#140](https://github.com/s9213712/hackme_web/issues/140) | 🟡 RECOMMEND | FEATURE_DEPENDENCY_RULES not enforced | [Recommend-7](PHASE_0_CLEANUP_GATE.md#recommend-7-github-140--feature_dependency_rules-not-enforced-) |
...
| 18 | [#141](https://github.com/s9213712/hackme_web/issues/141) | ⚪ LOW | test scans .venv site-packages | [Low-18](PHASE_0_CLEANUP_GATE.md#low-18-github-141--test-scans-venv-site-packages-) |
| 19 | [#142](https://github.com/s9213712/hackme_web/issues/142) | ⚪ LOW | prompt_password stdout pollution | [Low-19](PHASE_0_CLEANUP_GATE.md#low-19-github-142--prompt_password-stdout-pollution-) |
...
- [ ] **必修四件**：#122 / #135 / #136 / #137 全部 close + verification PASS
- [ ] **強烈建議併修**：#138 / #139 / #140（避免 Phase 1+ 重做）
- [ ] **既有持續項目**：#129 / #130 / #131 / PB-1 持續綠；silent fallback 全站 audit；incident_lockdown 跨路徑 hook
- [ ] **Low cleanup**：#141 / #142
- [ ] **Docs / 簽核**：ENGINEERING/QA §2 補 commit hash + tag；報告歸檔；root sign-off
...
> root 須在以下三選一：
>
> - **選項 A**（最保守，建議）：Phase 0 把 4 件 BLOCKER + 3 件 RECOMMEND 全部 close 再進 Phase 1
> - **選項 B**：4 件 BLOCKER 必修，3 件 RECOMMEND 帶進 Phase 1 但 Phase 1 出口前必須 close
> - **選項 C**（**不建議**）：只修 4 件 BLOCKER 即進 Phase 1，3 件 RECOMMEND 留到 Phase 7 啟用前 — settings/security gate 風險疊加進鏈化分支
...
| 7 | [../AGENTS/reports/claude/prechain_qa_2026-05-04/PRE_BLOCKCHAIN_READINESS_REPORT.md](../AGENTS/reports/claude/prechain_qa_2026-05-04/PRE_BLOCKCHAIN_READINESS_REPORT.md) | 知道 Phase 0 已做了哪些、還剩什麼（#122）|
...
```
Phase 0 ✅ (mostly done by codex follow-up; 唯 #122 仍 OPEN)
   │
Phase 1  ← 從這裡開始
   │
...
- [ ] 上一 phase 出口 gate 100% 過（`docs/BLOCKCHAIN/POINTSCHAIN_QA.md` 對應 §）
...
GATE_REPORT.md 模板可沿用 [`prechain_qa_2026-05-04/PRE_BLOCKCHAIN_READINESS_REPORT.md`](../AGENTS/reports/claude/prechain_qa_2026-05-04/PRE_BLOCKCHAIN_READINESS_REPORT.md) 的格式：Verdict / Branch / Commit / Coverage Matrix / Findings / Required Fixes。
...
3. 看 `docs/BLOCKCHAIN/` 是否已有「Phase N 實作完成」狀態標記
...
docs/research/    ← 設計討論草稿 (DRAFT)
├── POINTSCHAIN_V2_BLOCKCHAINIZATION_PLAN.md
└── POINTS_MINING_REWARDS_PLAN.md

docs/AGENTS/reports/<agent>/
├── prechain_qa_2026-05-04/                              ← Phase 0 base line
├── blockchain_design_2026-05-04/                        ← 鏈化設計 DRAFT 紀錄
├── mining_design_2026-05-04/                            ← 挖礦設計 DRAFT 紀錄
