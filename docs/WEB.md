...
- Spot trading bots are workflows owned by the user. The exchange page separates
  ...
  `/trading-workflow-editor.html` page. The first workflow engine supports
  branch priority, AND/OR conditions, cooldown, sequential action steps, price
  conditions, position checks, RSI/KD/MA/Bollinger conditions for backtests, and
  buy/sell/close-all actions. Backtests use the same workflow JSON and never
  place orders or mutate ledger state. Bot execution is manually scanned from
  the exchange page in this version to avoid unattended runaway trading.
