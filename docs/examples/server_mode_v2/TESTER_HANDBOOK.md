...
完整對照表：[`docs/SERVER_MODE_V2_PROFILE_MATRIX.md` §Token Types](../../SERVER_MODE_V2_PROFILE_MATRIX.md#token-types)
...
TESTER_USER=test \
TESTER_PW=testpw \
  bash docs/examples/server_mode_v2/01_internal_test_login_token.sh
```
...
TESTER_USER_ID=3 \
  bash docs/examples/server_mode_v2/02_tester_token_shadow_api.sh
```
...
TESTER_USER_ID=3 \
  bash docs/examples/server_mode_v2/06_full_feature_smv2.sh
```
...
ROOT_USER=root ROOT_PW='RootStrongP@ssw0rd' \
TESTER_USER_ID=3 \
  bash docs/examples/server_mode_v2/04_pentest_smv2.sh
```
...
ROOT_USER=root ROOT_PW='RootStrongP@ssw0rd' \
TESTER_USER_ID=3 \
  bash docs/examples/server_mode_v2/07_privilege_escalation_smv2.sh
```
...
**Why this matters**:
- 一兩句話說明這個行為違反 docs/SERVER_MODE_V2_PROFILE_MATRIX.md §<section> 的哪條
- 如果可被 chained 成更高權限：寫攻擊鏈

**Suggested test addition**:
- 應該補在 `tests/<existing_file>.py::test_<new_name>`
- 或 `docs/examples/server_mode_v2/04_pentest_smv2.sh` 加第 7 個 probe
```
...
| [`docs/SERVER_MODE_V2_PROFILE_MATRIX.md`](../../SERVER_MODE_V2_PROFILE_MATRIX.md) | 唯一權威的「哪個 mode 開哪個 feature」表 |
...
| `security/server_mode_v2_token_smoke.py` | 自己起 isolated runtime + 跑 01/02 的 smoke 工具 |
| `security/server_mode_v2_full_smoke.py` | 6 個腳本的 bundle 跑法 |
