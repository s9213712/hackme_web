...
>
> **依據**：[`docs/SERVER_MODE_V2_PROFILE_MATRIX.md §Production Gate Reports`](../../SERVER_MODE_V2_PROFILE_MATRIX.md#production-gate-reports) + `services/snapshots.py:35-49 PRODUCTION_REQUIRED_REPORT_TYPES`。

---

## 0. 前置認知

每份 report 必須符合 `services/snapshots.py:2786 upload_production_report` 的所有檢查，否則 reject：
...
|---|---|---|---|---|
| 1 | `clean_smoke` | Server Mode v2 乾淨 smoke：boot path / state-machine / 基線 endpoints | `security/server_mode_v2_clean_smoke.py` | JSON 結果檔 |
| 2 | `adversarial` | Mode v2 對抗測試：injection / bypass / mode-spoof | `security/server_mode_v2_adversarial.py` | JSON 結果檔 |
| 3 | `redteam_l2` | Mode v2 red-team Level 2 攻擊樹 | `security/server_mode_v2_redteam_l2.py` | JSON 結果檔 |
...
| 7 | `stress` | 流量 / trading 壓測 | `security/stress_test.py` + `security/trading_stress_pentest.py` | JSON 結果檔 |
| 8 | `permission` | role / permission pentest | `security/functional_permission_pentest.py` | JSON 結果檔 |
| 9 | `functional` | 全功能 smoke | `security/run_functional_smoke.sh` 或 `tests/smoke_suite.py` | JSON 結果檔 |
| 10 | `pentest` | 安全滲透測試 | `security/run_pentest.sh` + `security/session_security_pentest.py` | JSON 結果檔 |
| 11 | `snapshot_restore` | snapshot/restore regression | `tests/test_snapshots.py` 全綠 + 手動跑 1 次 create→restore→verify | JSON 結果檔 |
| 12 | `points_chain_consistency` | PointsChain 一致性 | `tests/test_points_chain.py` + `services/points_chain.verify_chain()` | JSON 結果檔 |
| 13 | `cloud_drive_quota_permission` | Cloud Drive quota & 權限 | `tests/test_cloud_drive_attachments.py` + `tests/test_storage_albums_schema.py` | JSON 結果檔 |
...
step clean_smoke    python3 security/server_mode_v2_clean_smoke.py
step adversarial    python3 security/server_mode_v2_adversarial.py
step redteam_l2     python3 security/server_mode_v2_redteam_l2.py
step pytest         python3 security/wrappers/pytest_to_report.py
step log_chain_verify  python3 security/wrappers/log_chain_verify.py
step integrity_guard   python3 security/wrappers/integrity_guard_check.py
step stress         python3 security/stress_test.py
step permission     python3 security/functional_permission_pentest.py
step functional     bash    security/run_functional_smoke.sh
step pentest        bash    security/run_pentest.sh
step snapshot_restore  python3 security/wrappers/snapshot_restore_check.py
step points_chain_consistency python3 security/wrappers/points_chain_verify.py
step cloud_drive_quota_permission python3 security/wrappers/cloud_drive_check.py
...
|---|---|---|
| `clean_smoke` | ✅ `security/server_mode_v2_clean_smoke.py` | 確認它的 `--out` flag 行為，產 JSON |
| `adversarial` | ✅ `security/server_mode_v2_adversarial.py` | 同上 |
| `redteam_l2` | ✅ `security/server_mode_v2_redteam_l2.py` | 同上 |
...
| `stress` | ✅ `security/stress_test.py` | 確認其 output 符合 §2 schema |
| `permission` | ✅ `security/functional_permission_pentest.py` | 同上 |
| `functional` | ✅ `security/run_functional_smoke.sh` | 同上 |
| `pentest` | ✅ `security/run_pentest.sh` | 同上 |
...
*Playbook end. 對應 spec：[`docs/SERVER_MODE_V2_PROFILE_MATRIX.md §Production Gate Reports`](../../SERVER_MODE_V2_PROFILE_MATRIX.md#production-gate-reports). 配套腳本：本資料夾 `01_internal_test_login_token.sh` + `02_tester_token_shadow_api.sh`。*
