...
- 音樂檔現在可直接在雲端硬碟 inline 預覽；若是從既有雲端影音檔發布影片，也可一起上傳自訂封面圖。prepared HLS 影片在 Safari 走原生 HLS，桌機 Chrome / Firefox / Edge 會自動切到內建 `hls.js`；strict E2EE 影音仍維持瀏覽器端解密播放，所以首次載入和快轉會較慢
