...
- 音樂檔現在可直接在雲端硬碟 inline 預覽；PDF 也會優先使用瀏覽器原生檢視器。若是 strict E2EE PDF，會先在瀏覽器端解密，再用內嵌檢視器或新分頁備援開啟。prepared HLS 影片在 Safari 走原生 HLS，桌機 Chrome / Firefox / Edge 會自動切到內建 `hls.js`；strict E2EE 影音仍維持瀏覽器端解密播放，所以首次載入和快轉會較慢
- 若你在同一次登入 session 內連續開多個 E2EE 檔案，系統會先嘗試你剛剛成功過的最近密碼；只有真的解不開時才會再跳出詢問
- 壓縮檔預覽現在會顯示檔案/資料夾清單、大小、壓縮後大小與備註，不再只是一段文字明細
- 若你發布的是 `持連結可看` 的 E2EE 影音，發布時只會在瀏覽器端要求你輸入一次原始 E2EE 密碼；之後可在影音頁的分享控制面板看到分享狀態、剩餘觀看次數、到期日、第二層分享密碼與重新產生 / 撤銷入口。若完整分享連結中的 fragment 遺失，伺服器無法幫你復原，只能重新產生分享
