...
```text
services/runtime/redis_client.py
services/experiments/job_state.py
```

設定：

```text
HTML_LEARNING_REDIS_URL=
HTML_LEARNING_EXPERIMENT_REDIS_ENABLED=0
```

Redis 只當 optional accelerator。沒有 Redis 時功能仍可用。
