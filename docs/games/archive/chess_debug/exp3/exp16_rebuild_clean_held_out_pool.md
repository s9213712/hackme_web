...
```bash
PYTHONPATH=<repo> python3 <repo>/scripts/games/chess_live_learning_validation.py --engines exp3 --quick-retrain-gate --output-root <chess_results>/exp16_clean_heldout_gate
```
...
最後一次 run 原始結果目錄：`<chess_results>/exp16_clean_heldout_gate`。
