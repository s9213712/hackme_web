...
```bash
PYTHONPATH=<repo> python3 <repo>/scripts/games/chess_live_learning_validation.py --engines exp2 --output-root <chess_results>/exp28_context_conditioned_flank_semantic_learning
```
...
最後一次 run 原始結果目錄：`<chess_results>/exp28_context_conditioned_flank_semantic_learning`。
