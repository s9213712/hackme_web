...
- 原始大型 artifact 仍以 `<chess_results>/...` 作為實跑來源路徑記錄；本資料夾不再保存 root/engine `summary.json` / `SUMMARY.md` 資料夾 dump。
...
- 實跑目錄：`<chess_results>/exp23_per_semantic_specialist_probe`。
- Specialist artifact：`<chess_results>/exp23_per_semantic_specialist_probe/exp3/semantic_specialist_probes.json`。
...
- Exp22 結果目錄：`<chess_results>/exp22_semantic_class_balanced_sampling`。
- Exp23 結果目錄：`<chess_results>/exp23_per_semantic_specialist_probe`。
...
- 修正版實跑目錄：`<chess_results>/exp25_balanced_gate_semantic_cleanup_v2`。
- 整體 verdict：`HIGH_RISK`，promotion 維持 false。
...
- `PYTHONPATH=<repo> python3 -m pytest tests/scripts/games/test_chess_live_learning_validation_script.py -q`：`27 passed in 279.82s`。
- `PYTHONPATH=<repo> python3 -m pytest tests/games -q`：`67 passed in 24.08s`。
- `git diff --check` 通過。
...
- 實跑目錄：`<chess_results>/exp27_flank_pawn_push_specialist_repair`。
- 整體 verdict：`HIGH_RISK`，promotion 維持 false。
...
- `python3 -m py_compile scripts/games/chess_live_learning_validation.py services/games/chess_dl.py` 通過。
- `PYTHONPATH=<repo> python3 -m pytest tests/scripts/games/test_chess_live_learning_validation_script.py -q`：`31 passed in 489.75s`。
- `PYTHONPATH=<repo> python3 -m pytest tests/games -q`：`67 passed in 23.83s`。
- quick deterministic gate + semantic specialist probes 實跑完成，artifact 寫入 `<chess_results>/exp27_flank_pawn_push_specialist_repair`。
...
- 結果目錄：`<chess_results>/exp28_context_conditioned_flank_semantic_learning`。
...
- `python3 -m py_compile scripts/games/chess_live_learning_validation.py services/games/chess_dl.py` 通過。
- `PYTHONPATH=<repo> python3 -m pytest tests/scripts/games/test_chess_live_learning_validation_script.py -q`：`34 passed in 492.33s`。
- `PYTHONPATH=<repo> python3 -m pytest tests/games -q`：`67 passed in 24.76s`。
- `git diff --check` 通過。
- quick deterministic gate + semantic specialist probes 實跑完成，artifact 寫入 `<chess_results>/exp28_context_conditioned_flank_semantic_learning`。
...
- 結果目錄：`<chess_results>/exp31_semantic_loss_budget_scheduler`。
- 扁平報告：
...
- quick deterministic gate：
  - `PYTHONPATH=<repo> python3 scripts/games/chess_live_learning_validation.py --engines exp3 --quick-retrain-gate --semantic-specialist-probes --seed 20260511 --quick-retrain-max-samples 220 --quick-retrain-max-seconds 120 --output-root <chess_results>/exp31_semantic_loss_budget_scheduler`。
...
- 結果目錄：`<chess_results>/exp33_failed_smoke_anchor_microdiagnosis_safe_checkpoint_retention`。
...
第一次同實驗 run 曾用 `<chess_results>/exp34_mixed_scheduler_repair_hard_case_decision_audit`，但因舊 trigger 直接啟動 full sanity gate，超過時間目標後中止；已合併記錄於扁平 exp34 報告。

實跑結果：

- 結果目錄：`<chess_results>/exp34_mixed_scheduler_repair_hard_case_decision_audit_fixed`。
