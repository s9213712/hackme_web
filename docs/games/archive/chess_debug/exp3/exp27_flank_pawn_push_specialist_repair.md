...
```bash
PYTHONPATH=<repo> python3 <repo>/scripts/games/chess_live_learning_validation.py --engines exp2 --semantic-specialist-probes --output-root <chess_results>/exp27_flank_pawn_push_specialist_repair
```
...
最後一次 run 原始結果目錄：`<chess_results>/exp27_flank_pawn_push_specialist_repair`。
