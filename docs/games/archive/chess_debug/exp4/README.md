...
- `<chess_results>/exp4_02_mcts_opening_multigood`
- `<chess_results>/exp4_03_retention_fixture_clean`
- `<chess_results>/exp4_04_probe_policy_fixed`
- `<chess_results>/exp4_06_full_sanity_sample64_final`（baseline reference）
- `<chess_results>/exp4_07_full_evidence_cleanup`（exp4_07 evidence cleanup）
- `<chess_results>/exp4_08_e_pawn_repair`（exp4_08 e_pawn equivalence audit + artifact size split）
- `<chess_results>/exp4_09_label_audit`（exp4_09 opening label audit + specialist gate cleanup）
- `<chess_results>/exp4_10_anti_poison`（exp4_10 anti-poison + special-rule + king-safety + override-search guard diagnostics）
- `<chess_results>/exp4_11_hardening`（exp4_11 quarantine wiring + chess_pv search guard + multi-good revoke）
- `<chess_results>/exp4_12_resign_draw_special`（exp4_12 求和 + 認輸 + 特殊走法擴充 + chess label features）
- `<chess_results>/exp4_12_audit_consistency`（exp4_12 audit/guard score-source 一致性）
- `<chess_results>/exp4_13_special_rule_curriculum`（exp4_13 special-rule + king-safety curriculum + 修 draw fixture）
- `<chess_results>/exp4_14_balanced_curriculum`（exp4_14 降權 + retention rehearsal + curriculum budget + rollback guard）
- `<chess_results>/exp4_15_rule_feature_consumption`（exp4_15 修 retention guard 假警報 + trainer 消費 rule_type + king-safety isolated probe + targeted rule before/after）
- `<chess_results>/exp4_16_rule_aware_final_fusion_locked`（exp4_16 rule-aware final fusion 鎖住 special-rule final move）
- `<chess_results>/exp4_vs_exp5_smoke_20260512_032501`（exp4_16 vs exp5_08 diagnostic sparring，非 promotion evidence）
- `<chess_results>/exp4_17_special_rule_subtype_consistency`（exp4_17 special-rule subtype + choose/explain consistency）
- `<chess_results>/exp4_17_gate_accounting_cleanup`（exp4_17 gate accounting cleanup）
- `<chess_results>/exp4_18_full_broad_learning_diagnostic`（exp4_18 full heavy sanity；確認 broad generalization 仍未通過）
- `<chess_results>/exp4_18_e_pawn_gate_accounting_fix`（exp4_18 e_pawn opening-book equivalence gate accounting fix）
- `<chess_results>/exp4_19_guarded_overlay_attribution`（exp4_19 baseline-default guarded overlay attribution）
- `<chess_results>/exp4_20_runtime_guarded_overlay`（exp4_20 no-label runtime guarded overlay simulator）
- exp4_21 runtime guarded overlay integration draft（預設關閉，尚未 promotion）
- `<chess_results>/exp4_22_actual_runtime_guarded_overlay_full`（exp4_22 actual runtime guarded overlay full diagnostic）
...
- quick targeted gate 已跑：
  - result dir：`<chess_results>/exp4_21_runtime_guarded_overlay_integration`
...
- result dir：`<chess_results>/exp4_22_actual_runtime_guarded_overlay_full`
- verdict：`HIGH_RISK`
- promotion：`false`
...
- `<chess_results>/exp4_17_special_rule_subtype_consistency`
- `<chess_results>/exp4_17_gate_accounting_cleanup`
...
- result dir：`<chess_results>/exp4_vs_exp5_smoke_20260512_032501`
- mode：6-game smoke，`diagnostic_only=true`
- exp4 model：`<chess_results>/exp4_16_rule_aware_final_fusion_locked/exp4/checkpoints/20/exp4_quick_candidate_model.json`
- exp5 model：`<chess_results>/exp5_08_stage_candidate/chess_experiment_5_nnue_stage_candidate.json`
...
- result dir：`<chess_results>/exp4_16_rule_aware_final_fusion_locked`
- verdict：`HIGH_RISK`
- promotion：`false`
