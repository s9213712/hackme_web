...
root 積分錢包應檢查「資金池管理」、「全用戶倉位管理」、Phase 1A 模擬經濟 fund summary 與 PointsChain 目前在外積分統計；這些是部署前確認資金池、用戶部位與鏈上供給口徑的唯讀入口。Phase 1A fund 數字必須來自 replay / derived cache verify，不可手填。交易費率、價格來源、回測容量、Bot 稽核、BTC_trade 整合與風控價格細節，都請改看深層文件。
