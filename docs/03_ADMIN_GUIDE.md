...
7.1. **回測 K 棒上限（`回測最大 K 棒數`）**：在 `交易所` 分頁的機器人區塊。
   預設 `20,000` 根；首次啟動伺服器時系統會在背景跑一輪 15 種 bot 的量測，
   把「本機 60 秒內最慢 bot 可跑的根數」自動填入這個欄位（不會覆蓋你已手動改過的值）。
   欄位旁的綠色 hint 會顯示量測結果（瓶頸 / 最快策略各別根數）。
   - 量測秒數預算（`回測極限量測秒數預算`）預設 `60` 秒，可調 `5–600`。
   - 上限上線範圍 `1,000 – 10,000,000`；保守起見建議 ≤ 量測值，否則最慢的範本
     在用戶 UI 上可能跑超時。
   - 重新量測：刪除 `trading.backtest_capacity_measured_at` 那筆 setting 後重啟（或
     呼叫 `trading_service.record_backtest_capacity_measurement(...)`）。詳見
     `docs/BACKTEST_CAPACITY_AND_TEMPLATE_BENCHMARKS.md`。
