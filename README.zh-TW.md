...
本地惡意程式掃描以免費、自架工具為核心。root 可啟用 `clamav` backend、
指定掃描器命令路徑、設定掃描逾時、掃描失敗是否 fail-closed、感染檔案是否
隔離，以及是否啟用副檔名/MIME magic-byte 檢查。若未指定命令，系統會優先
尋找 `clamdscan`，其次使用 `clamscan`。正式啟用前請在主機安裝 ClamAV 並用
`freshclam` 更新病毒碼。

對 server 可讀的 `public_attachment` 與 `private_scannable`，集中式掃描流程會
先做 magic-byte 檢查、zip archive safety 檢查，再執行 ClamAV；通過後才會從
`pending` 變成 `clean`。偵測到異常時預設標記為 `quarantined`。E2EE 檔案不會
宣稱完成伺服器端完整掃毒，因為伺服器無法解密內容。
