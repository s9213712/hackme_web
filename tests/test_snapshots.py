...
def test_runtime_reset_creates_pre_reset_snapshot_and_clears_runtime_tables_and_files(tmp_path):
    ...
    settings = {
        ...
        for row in conn.execute(
            "SELECT key, value FROM system_settings WHERE key LIKE 'feature_%' OR key IN ('allow_register', 'web_terminal_enabled', 'snapshot_daily_auto_enabled', 'storage_maintenance_auto_enabled')"
        ).fetchall()
    }
    conn.close()
    assert post_count == 0
    ...
    assert settings["web_terminal_enabled"] == "False"
