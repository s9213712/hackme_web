...
def test_server_mode_v2_full_smoke_references_extended_bundle_and_isolation_checks():
    ...
    token_smoke = (REPO_ROOT / "security" / "server_mode_v2_token_smoke.py").read_text(encoding="utf-8")
    ...
    assert 'REPO_ROOT / "docs" / "examples" / "server_mode_v2"' in script
    assert 'REPO_ROOT / "docs" / "examples" / "server_mode_v2"' in token_smoke
...
def test_full_feature_script_rotates_internal_test_token_with_target_username():
    script = (REPO_ROOT / "docs" / "examples" / "server_mode_v2" / "06_full_feature_smv2.sh").read_text(encoding="utf-8")
