...
**Current Release ID: `2026.05.05-110`**
...
回測共用同一份定義。
影音平台的大檔串流也已進入 Phase C-1：影片頁現在有 `playback` 決策 API、HLS
prepare/status 路由，以及 plain / `server_encrypted` 媒體的背景 HLS derivative
基礎；公開/持連結可看影片與伺服器端加密影音會優先自動準備 HLS 衍生檔，影片詳情頁也可手動重建或重試；不支援原生 HLS 的瀏覽器仍會自動退回既有直接串流。嚴格 `e2ee`
影音則維持瀏覽器端解密播放：擁有者在發布 `持連結可看` 的 E2EE 影音時，會在本地輸入一次原始 E2EE 密碼，把 file key 重新包成分享授權；觀看者只需要完整分享連結中的 fragment，
