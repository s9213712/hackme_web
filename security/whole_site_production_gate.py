...
def py_compile_all() -> Subcheck:
    ...
    for path in sorted(ROOT.rglob("*.py")):
        ...
        rel_text = str(rel)
        if any(part in rel.parts for part in excluded_parts) or rel_text.startswith("security/reports/"):
            continue
        checked += 1
        try:
...
def reports_output_policy_check() -> Subcheck:
    ...
    gitkeep = ROOT / "security" / "reports" / ".gitkeep"
    required = {
        "reports_dir_exists": (ROOT / "security" / "reports").exists(),
        "gitkeep_exists": gitkeep.exists(),
        "generated_reports_ignored": "security/reports/*" in gitignore and "!security/reports/.gitkeep" in gitignore,
    }
    return static_check(
        "security/reports output policy keeps generated gate reports out of git",
        all(required.values()),
        severity="MEDIUM",
        evidence=required,
        followup="Keep security/reports/.gitkeep tracked and generated report files ignored.",
    )
...
def build_modules(args) -> list[ModuleResult]:
    ...
    server_mode.add(script_check("security/server_mode_v2_clean_smoke.py", ["--out", str(out_dir)], timeout=args.timeout))
    server_mode.add(script_check("security/server_mode_v2_adversarial.py", ["--out", str(out_dir)], timeout=args.timeout))
    server_mode.add(script_check("security/server_mode_v2_redteam_l2.py", ["--out", str(out_dir)], timeout=args.timeout))
    server_mode.add(script_check("security/server_mode_v2_live_http_smoke.py", ["--out", str(out_dir)], timeout=max(args.timeout, 300)))
    ...
    rbac.add(pytest_check([
        ...
        "test_functional_permission_pentest.py",
        "test_security_issue_regressions.py",
    ], timeout=args.timeout))
    if args.base_url:
        rbac.add(run_command([
            sys.executable, str(ROOT / "security" / "functional_permission_pentest.py"),
    ...
    drive.add(pytest_check([
        ...
        "test_cloud_drive_attachments.py",
        "test_storage_maintenance.py",
        "test_storage_albums_schema.py",
        "test_remote_downloads.py",
    ...
    trading.add(pytest_check([
        ...
        "test_trading_workflow_editor_ui.py",
        "test_workflow_files.py",
    ], timeout=args.timeout))
    trading.add(script_check("security/trading_exchange_validation.py", ["--out", str(out_dir)], timeout=args.timeout))
    ...
    if args.base_url:
        trading.add(run_command([
            sys.executable, str(ROOT / "security" / "trading_stress_pentest.py"),
            "--base-url", args.base_url,
    ...
    forum.add(pytest_check([
        ...
        "test_moderation_proposals.py",
        "test_bug_reports.py",
        "test_sanction_notices.py",
    ], timeout=args.timeout))
