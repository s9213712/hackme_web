...
#     bash security/server_mode_v2_phase_5b_acceptance.sh
#
...
SMV2_TESTS=(
  tests/test_smv2_acceptance.py
  tests/test_db_mode_triggers.py
  tests/test_chain_production_only.py
  tests/test_smv2_context.py
  tests/test_routing_service.py
  tests/test_shadow_schema.py
  tests/test_cache_keys_namespace.py
  tests/test_trading_mode_gate.py
  tests/test_production_gate_enforcement.py
  tests/test_auth_csrf_safe.py
  tests/test_snapshots.py
  tests/test_points_chain.py
  tests/test_server_mode_v2_full_smoke_scripts.py
)
...
TRADING_TESTS=(
  ...
  tests/test_trading_websocket_inputs.py
...
run_block "A-4 live full smoke" "$PYTHON" security/server_mode_v2_full_smoke.py
...
hits=$(grep -nE 'INSERT INTO (trading_orders|trading_spot_positions|trading_margin_positions|points_ledger|wallets)\b' services/trading_engine.py || true)
if [ -z "$hits" ]; then
...
A6_TESTS=(
  tests/test_smv2_acceptance.py::test_tester_trade_does_not_change_production_wallet
  tests/test_smv2_acceptance.py::test_tester_trade_does_not_write_points_chain
  tests/test_smv2_acceptance.py::test_liquidation_does_not_cross_world
  tests/test_smv2_acceptance.py::test_funding_rate_does_not_cross_world
  tests/test_smv2_acceptance.py::test_matching_engine_namespaces_separate
)
