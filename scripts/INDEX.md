...
| `scripts/games/chess_exp5_validation_probe.py` | Chess validation | Games / ML | Run a 50-position held-out exp5 validation set and audit each exp5 decision with a local external Stockfish MultiPV teacher. | Selected validation JSON/JSONL under `docs/games/evidence/exp5/` or caller path | Stockfish is unavailable, validation questions are invalid, or candidate move quality regresses on held-out probes. |
