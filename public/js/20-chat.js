...
async function backupSelectedChatRoom() {
  if (!selectedChatRoomId) {
    setChatMsg("chat-room-warn", "請先選擇聊天室", false);
    return;
  }
  await fetchCsrfToken({ force: true });
  const csrf = getCsrfToken();
  const res = await apiFetch(API + `/chat/rooms/${encodeURIComponent(selectedChatRoomId)}/backup`, {
    credentials: "same-origin",
    headers: { "X-CSRF-Token": csrf || "" }
  });
  if (!res.ok) {
    const json = await res.json().catch(() => ({}));
    setChatMsg("chat-room-warn", json.msg || `備份聊天室失敗（HTTP ${res.status}）`, false);
    return;
  }
  const blob = await res.blob();
  const disposition = res.headers.get("Content-Disposition") || "";
  const filenameMatch = disposition.match(/filename="([^"]+)"/);
  const filename = filenameMatch ? filenameMatch[1] : `chat_backup_${selectedChatRoomId}.json`;
  ...
  setChatMsg("chat-room-warn", "聊天室備份已下載", true);
}

function chooseChatRestoreFile() {
  const input = $("chat-restore-file");
  if (!input) return;
  input.value = "";
  input.click();
}

async function restoreChatRoomBackup() {
  const input = $("chat-restore-file");
  const file = input?.files?.[0];
  if (!file) return;
  if (file.size > 2 * 1024 * 1024) {
    setChatMsg("chat-room-warn", "聊天室備份檔太大，最多 2 MiB", false);
    return;
  }
  const name = prompt("還原後的新聊天室名稱：", `還原 - ${file.name.replace(/\.json$/i, "")}`) || "";
  const form = new FormData();
  form.append("backup_file", file);
  if (name.trim()) form.append("room_name", name.trim());
  await fetchCsrfToken({ force: true });
  const csrf = getCsrfToken();
  const res = await apiFetch(API + "/chat/rooms/restore", {
    method: "POST",
    credentials: "same-origin",
    headers: { "X-CSRF-Token": csrf || "" },
    body: form
  ...
  const json = await res.json().catch(() => ({}));
  setChatMsg("chat-room-warn", json.msg || (json.ok ? "聊天室備份已還原" : `還原失敗（HTTP ${res.status}）`), !!json.ok);
  if (json.ok) {
    await loadChatRooms();
    if (json.room && json.room.id) await openChatRoom(json.room.id, true);
  }
}
